...
window.registerEvent = function (elem, evtName, fn, option) {
  ...
  if (evtName === "nocontextmenu") {
    ...
    if (window.isDef(fn)) {
      ...
      var events = getEventListeners(elem);if (window.isDef(events) && window.isDef(events[evtName])) {
        for (var j = 0; j < events[evtName].length; j++) {
          var e = events[evtName][j];elem.removeEventListener(evtName, e.listener);
        }
      }
    }
  }
};HTMLElement.prototype.on = function (evtName, fn, option) {
  var _this = this;var evts = evtName.split(" ");var cb = fn;var _loop = function _loop(i) {
    var evt = evts[i];if (_this.addEventListener) {
      if (!!document.documentMode && option && option.once) {
        self = _this;fn.removed = false;_oneCall = function oneCall() {
          if (!fn.removed) {
            fn();fn.removed = true;self.off(evt, _oneCall);
          }
        };cb = _oneCall;
    ...
    var self;var _oneCall;_loop(i);
  }
};HTMLElement.prototype.off = function (evtNames, fn) {
  var evts = evtNames.split(" ");for (var i = 0; i < evts.length; i++) {
    var _evt = evts[i];if (this.removeEventListener) {
  ...
  function registerLongMousedown(elem, fn) {
    var timer = 0;elem.on("mousedown", function (e) {
      var cb = function cb() {
        fn(e);
    ...
    var timer = 0;elem.on("keydown", function (e) {
      var cb = function cb() {
        fn(e);
      };timer = window.setTimeout(cb, 500);
  ...
  ev.stopImmediatePropagation();ev.cancelBubble = true;return false;
  ...
  var _iteratorNormalCompletion = true;var _didIteratorError = false;var _iteratorError = undefined;try {
    for (var _iterator = this[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var item = _step.value;item.on(evtName, fn, option);
    }
  } catch (err) {
  ...
  var _iteratorNormalCompletion2 = true;var _didIteratorError2 = false;var _iteratorError2 = undefined;try {
    for (var _iterator2 = this[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      var item = _step2.value;item.off(evtName, fn);
    }
  } catch (err) {
...
};window.on = window.addEventListener;document.on = document.addEventListener;window.off = window.removeEventListener;document.off = document.removeEventListener;window.isDef = function (obj) {
  return obj !== null && typeof obj !== "undefined";
};(function () {
  if (typeof window.CustomEvent === "function") {
    return false;
  }function CustomEvent(event, params) {
    params = params || { bubbles: false, cancelable: false, detail: undefined };var evt = document.createEvent("CustomEvent");evt.initCustomEvent(event, params.bubbles, params.cancelable, params.detail);return evt;
  }CustomEvent.prototype = window.Event.prototype;window.CustomEvent = CustomEvent;
})();document.fireEvent = function (eventName, detail) {
  var myEvent = new CustomEvent(eventName, { detail: detail });document.dispatchEvent(myEvent);
};
window.__SC = function () {
  ...
  var __registerEvents = [];

  function encodeShortcut(shortcutName) {
    shortcutName = shortcutName.toUpperCase();
    var humain = shortcutName;
    var ctrl = shortcutName.indexOf("CTRL+") >= 0;
    var shift = shortcutName.indexOf("SHIFT+") >= 0;
    var alt = shortcutName.indexOf("ALT+") >= 0;
    var meta = shortcutName.indexOf("META+") >= 0;
    shortcutName = shortcutName.replace("CTRL+", "").replace("SHIFT+", "").replace("ALT+", "").replace("META+", "");
    var k = keys[shortcutName] || shortcutName.charCodeAt(0);
    return { ctrl: ctrl, alt: alt, shift: shift, meta: meta, key: k, humain: humain };
  }

  function __registerEvent(el, shortcutName, fn, propage) {
    var enc = encodeShortcut(shortcutName);
    enc.propage = propage;
    enc.fn = fn;
    enc.target = el;
  ...
  function __unregisterEvent(el, shortcutName) {
    var humain = shortcutName.toUpperCase();
    for (var i = __registerEvents.length - 1; i >= 0; i--) {
      var sc = __registerEvents[i];
  ...
  function __onkeydown(el, event) {
    ...
    for (var i = 0; i < __registerEvents.length; i++) {
      ...
      if (e.target === el) {
        if (e.key === code && e.ctrl === event.ctrlKey && e.alt === event.altKey && e.meta === event.metaKey && e.shift === event.shiftKey) {
          __isPressing = true;
          if (!e.propage) {
